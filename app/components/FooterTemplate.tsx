...
export default function FooterTemplate() {
  ...
  const partners = [
    { name: 'TechCorp Studios', description: t('techPartner'), icon: Building2 },
    { name: 'Creative Nexus', description: t('creativePartner'), icon: Palette },
    { name: 'Digital Dreams Inc.', description: t('digitalPartner'), icon: Cpu },
    { name: 'Artistry Alliance', description: t('artPartner'), icon: Brush }
  ]
            ...
            <div className="space-y-2">
              {partners.map((partner) => (
                <div key={partner.name} className="flex items-center space-x-2 p-2 bg-white/5 rounded hover:bg-white/10 transition-colors">
                  <partner.icon className="h-4 w-4 text-magic-400" />
                  <div>
                    <p className="text-white font-exo2 font-semibold text-xs">
                      {partner.name}
                    </p>
