...
// マンガ関連の自動タグ生成
const generateMangaTags = (title: string, description: string, series: string): string[] => {
  const tags: string[] = []
  ...
  // マンガ関連のキーワード
  if (text.includes('original') || text.includes('オリジナル')) {
    tags.push('オリジナル')
  }
  if (text.includes('illustration') || text.includes('イラスト')) {
    tags.push('イラスト')
  }
  if (text.includes('comic') || text.includes('コミック')) {
    tags.push('コミック')
  }
  if (text.includes('webtoon') || text.includes('ウェブトゥーン')) {
    tags.push('ウェブトゥーン')
  }
  if (text.includes('manga') || text.includes('マンガ')) {
    tags.push('マンガ')
  }
  if (text.includes('fantasy') || text.includes('ファンタジー')) {
    tags.push('ファンタジー')
  }
  if (text.includes('romance') || text.includes('恋愛')) {
    tags.push('恋愛')
  }
  if (text.includes('action') || text.includes('アクション')) {
    tags.push('アクション')
  }
  if (text.includes('comedy') || text.includes('コメディ')) {
    tags.push('コメディ')
  }
  if (text.includes('drama') || text.includes('ドラマ')) {
    tags.push('ドラマ')
  }
  if (text.includes('mystery') || text.includes('ミステリー')) {
    tags.push('ミステリー')
  }
  if (text.includes('horror') || text.includes('ホラー')) {
    tags.push('ホラー')
  }
  if (text.includes('sports') || text.includes('スポーツ')) {
    tags.push('スポーツ')
  }
  if (text.includes('school') || text.includes('学園')) {
    tags.push('学園')
  }
  if (text.includes('isekai') || text.includes('異世界')) {
    tags.push('異世界')
  }
  if (text.includes('mecha') || text.includes('メカ')) {
    tags.push('メカ')
  }
  if (text.includes('slice of life') || text.includes('日常')) {
    tags.push('日常')
  }

  // デフォルトタグ
  if (tags.length === 0) {
    tags.push('マンガ', 'クリエイティブ')
  }

  return tags
}

export default function MangaPostPage() {
  ...
  useEffect(() => {
    if (postData.title || postData.description || postData.series) {
      const autoTags = generateMangaTags(postData.title, postData.description, postData.series)
      setSuggestedTags(autoTags.filter(tag => !postData.tags.includes(tag)))
    }
  }, [postData.title, postData.description, postData.series, postData.tags])
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-center gap-4 mb-8">
            ...
            <div>
              <h1 className="text-3xl font-bold text-white font-exo2">マンガ投稿</h1>
              <p className="text-white/70 font-exo2">マンガ作品をアップロードして共有しましょう</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-6 border border-white/10">
              <h2 className="text-xl font-semibold text-white mb-4 font-exo2">マンガ画像</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="md:col-span-2">
                  ...
                  <textarea
                    ...
                    placeholder="マンガの説明を入力"
                  />
                </div>

                {/* シリーズ名 */}
                <div>
                  <label className="block text-white/80 font-exo2 mb-2">シリーズ名</label>
                  <input
                    type="text"
                    value={postData.series}
                    onChange={(e) => setPostData(prev => ({ ...prev, series: e.target.value }))}
                    className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-lg text-white placeholder-white/50 focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-400/20 font-exo2"
                ...
                {/* チャプター */}
                <div>
                  <label className="block text-white/80 font-exo2 mb-2">チャプター</label>
                  <input
                    type="text"
                    value={postData.chapter}
                    onChange={(e) => setPostData(prev => ({ ...prev, chapter: e.target.value }))}
                    className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-lg text-white placeholder-white/50 focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-400/20 font-exo2"
                    placeholder="チャプター番号を入力（任意）"
                  />
                </div>
